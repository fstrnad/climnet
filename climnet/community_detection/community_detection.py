...
class BaseCommunityDetection():
    ...
    def __init__(self,
        ...
        if network is not None:
            self.net = network
            self.net.make_network_undirected(dense=dense_net)
            self.net.remove_isolated_nodes()
            if self.weighted:
    ...
    def reduce_group_levels(self, group_levels):
        ...
        red_group_levels = self.get_hierarchical_data_from_nodes(
            new_node_levels)
    ...
    def get_sorted_loc_gid(self, group_level_ids, lid):
        ...
        sorted_ids = [sorted(mean_lat_arr).index(i)
                      for i in mean_lat_arr]  # relative sorting
    ...
    def parallel_ordered_nl_loc(self, node_levels):
        ...
        for result_dict in parallelSortedLoc:
            ...
            loc_dict[lid] = {'lat': np.array(result_dict['sorted_lat']),
    ...
    def get_main_gr(self, region_dict, names, theta=None):
        if theta is None:
            theta = self.theta
        ...
        for region in names:
            ...
            all_gr_numbers = np.concatenate((all_gr_numbers, gr_numbers),
                                            axis=0)
        ...
        if rel_freq < 0.5:
            print(f'WARNING! Less than half of all rep ids is in community: \
