...
class BaseDataset(bds.BaseDataset):
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if load_nc is not None:
            ...
            # set to grid type with points as index
            self.grid = self.init_grid(
                ds, grid_step=grid_step,
                grid_type=grid_type, **kwargs)
            # regridding to Climnet Grid (eg. Fekete)
            self.ds = self.interp_grid(ds, self.grid)

            if large_ds is True:
                ds.unify_chunks()

        # Init mask
        self.mask = self.init_mask(da=self.ds[self.var_name],
                                   init_mask=True,  # to init key_val_idx_point_dict
                                   **kwargs)

        # select a main var name
        self.vars = gut.get_vars(self.ds)
    ...
    def init_grid(self, ds, grid_step=1,
        ...
        elif grid_type == "fekete" or grid_type == 'Fekete':
    ...
    def interp_grid(self, dataarray, new_grid):
        ...
        if isinstance(dataarray, xr.Dataset):
            dataarray = dataarray[self.var_name]
        ...
        if len(dataarray.data.shape) < 3:
            ...
            if self.grid_type != 'gaussian':
                ...
                # if already on gaussian grid do not interpolate but use the grid
                gut.myprint(f'No interpolation needed for gaussian grid!')
                data_flattened = dataarray.stack(points=('lon', 'lat'))
                new_points = data_flattened.points.data
                coordinates = sput.create_new_coordinates(
                    times=dataarray.time.data,
                    lons=data_flattened.lon.values, lats=data_flattened.lat.values
                )
                new_values = data_flattened.data
        ...
        new_dataarray = new_dataarray.to_dataset(name=self.var_name)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "anomalies" in self.vars:
            ...
            self.compute_anomalies_ds(an_types=[climatology])
