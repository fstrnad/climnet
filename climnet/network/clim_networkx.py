...
class Clim_NetworkX:
    ...
    def __init__(
        self, dataset, nx_path_file=None, network=None,
        var_name=None,
        corr_matrix=None, **kwargs
    ):

        self.ds = dataset
        self.ds.var_name = var_name if var_name is not None else self.ds.var_name
        self.adjacency = None
        self.corr = None
        ...
        if network is not None or corr_matrix is not None:
            self.create(network=network, corr_matrix=corr_matrix, **kwargs)
        elif nx_path_file is None:
            return
        else:
            self.cnx = self.load(nx_path_file)

    def create(self, method='corr', network=None,
               corr_matrix=None,
               pvalue=None, weighted=False, **kwargs):

        self.weighted = weighted
        if network is not None:
            gut.myprint(f'Create Network based on input network')
            self.adjacency = network.adjacency
            self.corr = np.where(self.adjacency == 1, network.corr, 0)
        else:
            if corr_matrix is not None:
                gut.myprint(f'Create Network based on input corr matrix!')
                self.corr = corr_matrix
                self.pvalue = pvalue
                self.adjacency = cnet.get_adjacency_corr(
                    ds=self.ds,
                    corr=self.corr,
                    pvalue=self.pvalue,
                    **kwargs
                ...
                gut.myprint(f'Create Network based on method {method}')
                if method == 'corr':
                    reload(cnet)
    ...
    def save(self, savepath, ds_savepath=None,
             set_rempoints=True,
             cnx=None):
    ...
    def set_corr_edges(self):
        ...
        gut.myprint("Finished setting edge corr values!")
    ...
    def unset_removed_points(self):
        nnodes = len(self.cnx.nodes)
        for n in range(nnodes):
            n_attrs = list(self.cnx.nodes[n].keys)
            if 'rempoints' in n_attrs:
                del self.cnx.nodes[n]['rempoints']
        return
    ...
    def compute_curvature(self, c_type="forman",
        ...
        tt = time.time()
        if self.weighted:
            gut.myprint("ATTENTION! Weighted is True!")
        ...
        if self.node_attr_exists(attr=c_type) and rc_curv is False:
            ...
            gut.myprint(f"time elapsed: {time.time() - tt}")
    ...
    def get_edges_between_regions(
        self, lon_range_s, lat_range_s,
        lon_range_t=[-180, 180],
        lat_range_t=[-90, 90],
        attribute=None, binary=False, q=None,
        ...
        if bi_directional:
            ...
            el_st = []
            for e in link_dict_s['el']:
                sid, tid = e
                t_dict = self.get_single_node(tid)
                tlon, tlat = t_dict['lon'], t_dict['lat']
                if sput.in_lon_lat_range(lon=tlon, lat=tlat,
                                         lon_range=lon_range_t,
                                         lat_range=lat_range_t):
                    el_st.append(e)
        el_st = np.array(el_st)
        num_link_arr = self.ds.count_indices_to_array(el_st.flatten())  # All edges are counted, source-target does not matter
        if binary:
            num_link_arr = np.where(num_link_arr == 0, np.nan, 1)
        else:
            num_link_arr[np.where(num_link_arr == 0)[0]] = np.nan
