...
class BaseGrid():
    ...
    def cut_grid(self, lon_range, lat_range):
        ...
        if lon_range[0] > lon_range[1]:
            ...
            print(f"grid in range lat: {lat_range} and lon: {lon_range}")
...
class GaussianGrid(BaseGrid):
    ...
    def __init__(self, grid_step_lon, grid_step_lat, grid=None, da=None):
        super().__init__()
    ...
    def create_grid(self):
        ...
        if self.da is not None:
            if isinstance(self.da, xr.DataArray) or isinstance(self.da, xr.Dataset):
                lons, lats = xr.broadcast(da.lon, da.lat)
                lon_mesh = lons.values
                lat_mesh = lats.values
        else:
...
class FeketeGrid(BaseGrid):
    ...
    def __init__(self, num_points, num_iter=1000,
        ...
        gut.myprint(f'Try to load grid {load_grid}...')
        grid_path = f'{self.savefolder}/{load_grid}'
        if load_grid is None or not fut.exist_file(grid_path):
            if load_grid is not None:
                gut.myprint(f'{grid_path} does not exist. Create grid...')
