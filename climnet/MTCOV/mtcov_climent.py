...
import os
import time
from climnet.dataset import BaseDataset, EvsDataset
from climnet.net import EventSyncClimNet, load
from climnet.community_detection.community_detection import BaseCommunityDetection
import climnet.MTCOV.MTCOV as mtcov
import numpy as np
...
class MTCOV_Climnet(BaseCommunityDetection):
    ...
    def __init__(self,
        ...
        self.A, self.B, self.nodes = self.prepare_graph(adjacency=self.net.adjacency,
                                                        weights=self.net.corr,
    ...
    def run_mtcov(self,
        ...
        MTCOV = mtcov.MTCOV(N=self.A[0].number_of_nodes(),
                            L=len(self.A),
                            C=C,
                            Z=self.Xs.shape[1],
                            gamma=gamma,
        ...
        print("\nTime elapsed:",
              np.round(time.time() - time_start, 2), " seconds.")
        ...
        self.hard_cluster = self.get_hard_cluster(theta=self.theta,
                                                  ordered=True)
    ...
    def prepare_graph(
        ...
        print('\nAdjacency shape: {0}'.format(adjacency.shape))

        # create the graph adding nodes and edges
        A = self.read_graph(adj=adjacency,
                            weights=weights,
                            weighted=weighted,
                            undirected=undirected,
    ...
    def print_graph_stat(self, A):
        ...
        if not unweighted:
            print('Average edges over all layers (weighted):',
                  np.round(avg_M / L, 3))
            print('Average degree over all layers (weighted):',
    ...
    def get_hard_cluster(self, theta, ordered=True):
        ...
        hard_cluster = self.reduce_node_levels([first_clustering])
        ...
        if ordered:
            ...
            self.theta = self.reorder_theta(
                theta, loc_dict[0]['ids'])  # Zero for level id
    ...
    def compute_prob_map(self, sp_arr,
                         all_region_dict,
                         arr_regions):
        collect_res = []
        gr_maps = []
        for run, sp_theta in enumerate(sp_arr):
            ...
            if os.path.exists(sp_theta):
                ...
                theta = self.load_communities(sp_theta)
                gr_num = self.get_main_gr(all_region_dict,
                                          arr_regions,
                                          theta=theta)
